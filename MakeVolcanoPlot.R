MakeVolcanoPlot <- function(data_location,group_divisions,group_designations_scheme,genes_to_label_file=NULL)
{
...
genes_to_label <- NULL
if (is.null(genes_to_label_file)) {genes_to_label <- c('MTHFR')}

if (!is.null(genes_to_label_file))
{
  genes_to_label_df <- read.table(file=genes_to_label_file,head=FALSE,check.names=FALSE,sep='\t')
...
patient_names <- colnames(DATA) #store the patient names
